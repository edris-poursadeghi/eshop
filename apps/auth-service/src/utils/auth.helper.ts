...
export const trackOtpRequests = async (email: string, next: NextFunction) => {
  ...
  if (otpRequests > 2) {
    ...
    return next(
      new ValidationError(
        'Too many OTP requests. Please wait 1 hour abefore requesting agian.'
      )
    );
  }

  await redis.set(otpRequestKey, otpRequests + 1, 'EX', 3600); // Tracking requests for 1h
};
...
export const verifyForgotPasswordOtp = async (
  ...
  try {
    ...
    res
      .status(200)
      .json({ mssage: 'OTP verifyed. You can now reset your password.' });
  } catch (error) {
    next(error);
  }
};
